...
export class DataService extends BaseService {
  ...
  get<T>(action?: string, id?: number | string): Observable<T> {
    ...
    if (action) action = '/' + action; else action = '';
    if (id==undefined||id==null) id = '';
    return this.http.get(this._baseUrl + `/${this._controller}${action}/${id}`, { headers })
      .map(response => response.json())
      .catch(this.handleError);
  }

  getPage<T>(action?: string, qso?: QueryStringOptions): Observable<Pagination<T>> {
    action = action || this._action;
    qso = Object.assign({
      column: '',
      page: 1,
    ...
    const url = `/${this._controller}${action}/?sortBy=${qso.sortBy}&page=${page}&size=${qso.size}` +
      `&column=${qso.column}&query=${qso.query}&userId=${qso.userId}`;

    return this.http.get(this._baseUrl + url, { headers })
      .map(response => response.json())
      .catch(this.handleError);
  }
  ...
  update(data: any, action: string = ''): Observable<boolean> {
    ...
    action = action || this._action;
    return this.http.put(this._baseUrl + `/${this._controller}/${action}`, JSON.stringify(data), options)
      .map(res => true)
      .catch(this.handleError);
  }

  delete(id: number|string, action: string = ''): Observable<boolean> {
    const options = new RequestOptions({ headers: this.getHeaders() });
    action = action || this._action;
    if (action) action = action + '/';
    return this.http.delete(this._baseUrl + `/${this._controller}/${action}${id}`, options)
      .map(res => true)
      .catch(this.handleError);
  }
  ...
  send(method: string, data: any, action?: string) {
    ...
    switch (met) {
      ...
      default:
        throw "Only methods 'POST' and 'UPDATE' (or 'PUT') are supported.";
    }
  }
